...
// Only these addresses are allowed to deploy smart contracts. This
// prevents the chain from getting bloated due to random stuff getting
// deployed.
var whitelisted_addresses = map[string]bool {
	"0x35b357a01cd1eddbf7dfdf656f0fba9ad0a84464": true, // Marconi Foundation Address
...
func ApplyTransaction(config *params.ChainConfig, bc ChainContext, author *common.Address, gp *GasPool, statedb *state.StateDB, header *types.Header, tx *types.Transaction, usedGas *uint64, cfg vm.Config) (*types.Receipt, uint64, error) {
	...
	// if the transaction wants to create a contract, and the address whitelist is enabled...
	if msg.To() == nil && config.ContractsRequireWhitelistedAccount {
		// only allow it if the sender address is whitelisted.
		if whitelisted_addresses[hexutil.Encode(msg.From().Bytes())] == false {
			return nil, 0, errors.New("Sender address isn't whitelisted")
		}
	}
